...
import { Play, Pause, Download, Video, Trash2, ArrowDownToLine, RefreshCw, Maximize2, VolumeX, Volume2, Info, Globe, Lock, Heart, ImageIcon } from 'lucide-react';
import { AiArtGallery } from './ai-art-gallery';
...
interface MusicPlayerCardProps {
    ...
    onPlay?: () => void; // Optional play callback (e.g. for playlist context)
    imageUrl?: string; // Cover image URL
    generatedVideoImages?: string[]; // AI art images from video creation
    trackOwnerId?: string; // Track owner UID for gallery ownership check
    onShowBuyCoins?: () => void; // Open buy coins modal
}
...
export function MusicPlayerCard({
    ...
    const hasGallery = generatedVideoImages.length > 0 || (videoUrl && videoStatus === 'completed');
    ...
    const handleBuyTrack = async () => {
        if (!onShowBuyCoins) return; // Need this to show modal if lacking coins
        
        const confirmBuy = window.confirm(
            "Buy Exclusive Rights for 25 coins?\n\nThis will transfer full ownership to you and remove the track from the public community forever."
        );
        ...
        try {
            ...
            const res = await fetch('/api/rraasi-music/buy-track', {
                method: 'POST',
                headers: {
            ...
            if (!res.ok) {
                ...
                throw new Error(data.error || 'Failed to buy track');
            }

            alert("Success! The track is now exclusively yours.");
            if (onSync) onSync();
        } catch (error: any) {
            console.error('Failed to buy track:', error);
            alert(error.message);
        } finally {
    ...
    */
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 flex h-64 flex-col justify-between p-5 text-white pointer-events-none">
                ...
                <div className="relative z-30 flex items-center justify-between pointer-events-auto">
                    <div className="flex items-center gap-1.5">
                        ...
                        {hasGallery && (
                            <button
                                ...
                                className="rounded-full bg-purple-500/30 hover:bg-purple-500/50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider backdrop-blur-md shadow-sm text-purple-100 flex items-center gap-1 border border-purple-400/20 transition-all hover:scale-105"
                                title="View AI Art Gallery"
                            >
                                <ImageIcon className="w-3 h-3" />
                                {generatedVideoImages.length > 0 ? generatedVideoImages.length : ''} AI Art
                            </button>
                        )}
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        />
                        {isPublic && profile?.uid !== trackOwnerId && profile?.uid && !isOwner && (
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                    handleBuyTrack();
                                }}
                                className="flex items-center gap-1.5 h-8 px-3 rounded-full bg-gradient-to-r from-amber-500 to-amber-400 hover:from-amber-400 hover:to-amber-300 text-black font-bold text-[11px] uppercase tracking-wider transition-all shadow-[0_0_10px_rgba(245,158,11,0.4)] hover:scale-105 mx-1"
                                title="Buy Exclusive Commercial Rights"
                                disabled={isPurchasing}
                            >
                                {isPurchasing ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Coins className="w-3.5 h-3.5" />}
                                Buy - 25
                            </button>
                        )}
                        <TrackActionsMenu
            ...
            {/* AI Art Gallery Modal */}
            {hasGallery && (
                <AiArtGallery
                    isOpen={showGallery}
                    onClose={() => setShowGallery(false)}
                    trackTitle={displayTitle}
                    trackId={shareId?.split('?')[0] || trackId}
                    trackOwnerId={trackOwnerId}
                    images={generatedVideoImages}
                    videoUrl={videoUrl}
                    prompt={description || prompt}
                    onShowBuyCoins={onShowBuyCoins}
                />
            )}
