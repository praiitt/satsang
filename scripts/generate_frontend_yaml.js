...
const envs = {
    ...
    LIVEKIT_EGRESS_GCP_CREDENTIALS: JSON.stringify(serviceAccount)
};

const yamlContent = Object.entries(envs).map(([k, v]) => `${k}: '${v.replace(/'/g, "'\\''")}'`).join('\n');

fs.writeFileSync(path.join(__dirname, '../frontend-env.yaml'), yamlContent);
console.log('Generated frontend-env.yaml');
