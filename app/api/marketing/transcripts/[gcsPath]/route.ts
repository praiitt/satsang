...
export async function GET(
  request: NextRequest,
  context: { params: Promise<{ gcsPath: string | string[] }> }
) {
  const params = await context.params;
  // Handle both single segment and multiple segments (catch-all)
  const gcsPathArray = Array.isArray(params.gcsPath) ? params.gcsPath : [params.gcsPath];
  const gcsPath = gcsPathArray.join('/'); // Reconstruct the full GCS path
