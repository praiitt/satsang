...
router.post('/import-charts', async (req, res) => {
  try {
    ...
    await hybridRAGService.storeUserProfile(userId, {
      name: birthData.name,
      birthData: JSON.stringify(birthData)
    });

    // Store chart data in hybrid RAG service
    await hybridRAGService.storeUserChartData(userId, chartResults.data);
...
router.post('/import-contact-charts', async (req, res) => {
  try {
    ...
    // Store contact profile in hybrid RAG service
    try {
      ...
      // If contact already exists, just update the chart data
      logger.info('Contact already exists, updating chart data only', { userId, contactName });
    }

    // Update contact chart data in hybrid RAG service
    await hybridRAGService.updateContactChartData(userId, contactName, chartResults.data);
    ...
    logger.error('Error in contact chart import:', error);
    res.status(500).json({
      success: false,
      error: 'Internal server error',
      message: 'Failed to import contact charts'
    });
  }
});
...
router.get('/user-charts/:userId', async (req, res) => {
  try {
    ...
    const chartResult = await hybridRAGService.getAllUserCharts(userId);
...
router.get('/contact-charts/:contactId', async (req, res) => {
  try {
    ...
    logger.info('Retrieving contact charts', { contactId });

    // Get actual chart data from hybrid RAG service
    const chartResult = await hybridRAGService.getAllUserCharts(contactId);
    ...
    res.status(500).json({
      success: false,
      error: 'Internal server error',
      message: 'Failed to retrieve contact charts'
    });
  }
});
...
router.get('/user-charts/:userId', async (req, res) => {
  try {
    ...
    const userCharts = await hybridRAGService.getAllUserCharts(userId);
...
router.post('/auto-import-missing-charts/:userId', async (req, res) => {
  try {
    ...
    const userProfile = await hybridRAGService.getUserProfile(userId);
    ...
    const currentCharts = await hybridRAGService.getAllUserCharts(userId);
    ...
    if (successfulImports.length > 0) {
      await hybridRAGService.importExistingChartsToRAG(userId);
    }

    // Check final status
    const finalCharts = await hybridRAGService.getAllUserCharts(userId);
