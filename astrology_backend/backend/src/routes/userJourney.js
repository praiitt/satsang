...
// Get user profile for journey tracking
router.get('/profile/:userId', async (req, res) => {
  try {
    const { userId } = req.params;
    
    logger.info('Getting user profile for journey tracking', { userId });

    // Import firestoreRAGService dynamically to avoid circular dependencies
    const { firestoreRAGService } = await import('../services/firestoreRAGService.js');
    
    // Get user profile and charts
    ...
    // Get recent conversations/activities
    const recentConversations = await firestoreRAGService.getRecentConversations(userId, 5);

    const profileData = {
      userId,
      profile: userProfile.profile,
      charts: userCharts.success ? userCharts.charts : [],
      totalCharts: userCharts.success ? userCharts.totalCharts : 0,
      recentActivity: recentConversations.success ? recentConversations.conversations : [],
      journeyStatus: {
        hasProfile: !!userProfile,
        hasCharts: userCharts.success && userCharts.totalCharts > 0,
        isActive: recentConversations.success && recentConversations.conversations.length > 0
      },
      timestamp: new Date().toISOString()
    };
    ...
    logger.error('Error getting user profile for journey tracking:', error);
    res.status(500).json({
      success: false,
      error: 'Internal server error',
