...
router.post('/store-user-profile', async (req, res) => {
  try {
    ...
    const result = await firestoreRAGService.storeUserProfile(userId, userData);
...
router.post('/get-chart-for-query', async (req, res) => {
  try {
    ...
    const result = await firestoreRAGService.retrieveRelevantCharts(userId, query);
...
router.get('/user-profile/:userId', async (req, res) => {
  try {
    ...
    const profile = await firestoreRAGService.getUserProfile(userId);
...
router.get('/stats', async (req, res) => {
  try {
    const stats = await firestoreRAGService.getDatabaseStats();
...
router.post('/import-charts-to-rag/:userId', async (req, res) => {
  try {
    ...
    const result = await firestoreRAGService.importExistingChartsToRAG(userId);
...
router.get('/user-contacts/:userId', async (req, res) => {
  try {
    ...
    const result = await firestoreRAGService.getUserContacts(userId);
    ...
    logger.info('Retrieved user contacts', { userId, contactCount: result.contacts.length });
    ...
    res.status(500).json({
      success: false,
      error: 'Failed to retrieve user contacts'
    });
  }
});

// Test contact lookup
router.get('/test-contact/:userId/:contactName', async (req, res) => {
  try {
    const { userId, contactName } = req.params;
    
    logger.info('Testing contact lookup', { userId, contactName });
    
    const result = await firestoreRAGService.getContactByName(userId, contactName);
    ...
    logger.error('Error testing contact lookup:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to test contact lookup'
    });
  }
});

// Delete a contact
router.delete('/user-contacts/:userId/:contactName', async (req, res) => {
  try {
    const { userId, contactName } = req.params;
    
    logger.info('Deleting contact', { userId, contactName });
    ...
    logger.info('Contact deleted successfully', { userId, contactName, deletedCount: result.deletedCount });
...
router.post('/user-contacts/:userId', async (req, res) => {
  try {
    ...
    const result = await firestoreRAGService.storeUserContact(userId, contactData);
...
router.put('/user-contacts/:userId/:contactName/chart-data', async (req, res) => {
  try {
    ...
    const result = await firestoreRAGService.updateContactChartData(userId, contactName, chartData);
